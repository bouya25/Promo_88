					...
					if( 0 < $pics_v_n || 0 < $pics_v_local_n ) {
						...
						if( 0 < $pics_v_n ) {
							for( $i = 0; $i < $pics_v_n; $i ++) {
								...
								$comment->comment_content .= '<img class="cr-comment-videoicon" src="' . plugin_dir_url( __FILE__ ) . 'img/video.svg">';
								$comment->comment_content .= '</div>';
								$k++;
							}
						}
				...
				<div class="cr-ajax-reviews-cus-images-modal">
					...
					<div class="cr-ajax-reviews-cus-images-slider-main cr-reviews-slider" data-slick='<?php echo wp_json_encode( $main_slider_settings ); ?>'>
						...
						for( $i = 0; $i < $count; $i ++) {
							...
							$output .= '<p class="cr-ajax-reviews-slide-main-comment-body">' . get_comment_text( $pics_prepared[$i][1]->comment_ID ) . '</p>';
							if( $voting_enabled ) {
								ob_start();
