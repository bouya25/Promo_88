		...
		if ( 0 < $pics_v_n || 0 < $pics_v_local_n ) {
			...
			if ( 0 < $pics_v_n ) {
				for ( $i = 0; $i < $pics_v_n; $i++ ) {
					echo '<div class="cr-comment-video cr-comment-video-' . ($i + 1) . '">';
					echo '<video preload="metadata" class="cr-video-a" ';
					echo 'src="' . $pics_v[$i]['url'];
					echo '" alt="' . sprintf( __( 'Video #%1$d from %2$s', 'customer-reviews-woocommerce' ), $i + 1, $comment->comment_author ) . '"></video>';
			...
			if ( 0 < $pics_v_local_n ) {
				...
				for ( $i = 0; $i < $pics_v_local_n; $i++ ) {
					...
					if ( $attachmentUrl ) {
						$temp_comment_content_flag = true;
						$temp_comment_content .= '<div class="cr-comment-video cr-comment-video-' . ($i + 1) . '">';
						$temp_comment_content .= '<video preload="metadata" class="cr-video-a" ';
						$temp_comment_content .= 'src="' . $attachmentUrl;
						$temp_comment_content .= '" alt="' . sprintf( __( 'Video #%1$d from %2$s', 'customer-reviews-woocommerce' ), $i + 1, $comment->comment_author ) . '"></video>';
