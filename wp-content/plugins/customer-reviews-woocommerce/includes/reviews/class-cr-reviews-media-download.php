...
if ( ! class_exists( 'CR_Reviews_Media_Download' ) ) :
	...
	class CR_Reviews_Media_Download {
		...
		private static function auto_download_media() {
			...
			if( !$transientExists ) {
				...
				if( 0 < $mediaCount ) {
					...
					echo '<p>' . sprintf( __( 'There are %d media file(s) attached by customers to reviews but not downloaded into WordPress Media Library yet.', 'customer-reviews-woocommerce' ), $mediaCount ) . '</p>' ;
		...
		private function download_media_file( $mediaFileToDownload, $downloadURL, $commentMeta ) {
			...
			//
			$tmpFile = download_url( $downloadURL['url'] );
			$file_array = array(
			...
			if ( is_wp_error( $tmpFile ) ) {
				...
				if( false !== strpos( $tmpFile->get_error_code(), '404' ) ) {
